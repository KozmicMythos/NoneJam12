//NOVO MAPA
if mostra_mapa {
    var mini_tela_w = tela_size * escala;
    var mini_tela_h = tela_altura * escala;
    ...
    var offset_x = tela_central_x * mini_tela_w - display_get_gui_width()/2 + mini_tela_w/2;
    var offset_y = tela_central_y * mini_tela_h - display_get_gui_height()/2 + mini_tela_h/2;
            ...
            if descoberto[tx][ty] {
                ...
                for (var i = 0; i < tiles_x; i++) {
                    for (var j = 0; j < tiles_y; j++) {
                        ...
                        if tile != 0 {
                            var draw_x = real_x * mini_tela_w - offset_x;
                            var draw_y = real_y * mini_tela_h - offset_y;

                            // Checa se está visível na GUI
                            if draw_x + mini_tela_w > 0 && draw_y + mini_tela_h > 0 &&
                               draw_x < display_get_gui_width() && draw_y < display_get_gui_height() {
                                draw_set_color(cor); 
                                draw_rectangle(draw_x, draw_y, draw_x + mini_tela_w, draw_y + mini_tela_h, false);
    ...
    if instance_exists(obj_player) {
        ...
        if descoberto[tela_nave_x][tela_nave_y] {
            ...
            //draw_sprite(spr_nave,image_index,nx,ny);
            draw_sprite_ext(spr_player,image_index,nx,ny,image_xscale * escala,image_yscale * escala,image_angle,image_blend,image_alpha)
